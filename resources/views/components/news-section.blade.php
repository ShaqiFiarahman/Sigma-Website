        ...
        @forelse($news as $item)
            <div onclick="window.open('{{ $item['url'] ?? '#' }}', '_blank')"
                ...
                <div class="relative h-32 overflow-hidden">
                    ...
                    @endif
                    <div class="absolute top-3 left-3 bg-white/90 text-blue-800 text-[10px] font-bold px-2.5 py-1.5 rounded-lg uppercase">
                        {{ $item['source'] ?? 'BERITA' }}
                    </div>
                </div>
                <div class="p-5 flex-1 flex flex-col justify-between">
                    <div>
                        <p class="font-bold text-sm leading-relaxed line-clamp-2 text-slate-900 mb-3">{{ $item['title'] ?? '-' }}</p>
